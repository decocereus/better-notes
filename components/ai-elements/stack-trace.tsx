...
const isInternalPath = (path: string | null | undefined): boolean => {
	if (!path) {
		return false;
	}
	return (
...
const parseLineNumber = (value: string | undefined): number | null =>
	value ? Number.parseInt(value, 10) : null;

const buildStackFrame = (
	raw: string,
	functionName: string | null,
	filePath: string | null,
	...
	functionName,
	filePath,
	lineNumber: parseLineNumber(lineNum),
	columnNumber: parseLineNumber(colNum),
	isInternal: isInternalPath(filePath) || isInternalPath(raw),
});
...
const parseStackFrame = (line: string): StackFrame => {
	...
	if (withoutFnMatch) {
		...
		return buildStackFrame(trimmed, null, filePath ?? null, lineNum, colNum);
	}

	// Fallback: unparseable line
	return buildStackFrame(trimmed, null, null, undefined, undefined);
};
